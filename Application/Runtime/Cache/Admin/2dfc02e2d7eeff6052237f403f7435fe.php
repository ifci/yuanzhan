                    ...
                    <form>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
                            ...
                            <tr>
                                ...
                                <td><?php if(ACTION_NAME == 'add')$info['status'] = 1; ?><label><input type="radio" name="info[status]" value="1" <?php if($info["status"] == 1): ?>checked="checked"<?php endif; ?> /> 文章已发布状态</label> &nbsp;<label><input type="radio" name="info[status]" value="0" <?php if($info["status"] == 0): ?>checked="checked"<?php endif; ?> /> 文章审核状态</label></td>
                            </tr>
                            <tr>
                                <th>文章所属分类：</th>
                                <td>
            ...
            $(function(){var  content ;
                KindEditor.ready(function(K) {
                    content = K.create('#content',{
                        ...
                        uploadJson:'/yuanzhan/Public/kindeditor/php/upload_json.php?dirname=news'
                    });
                });
                $("#checkNewsTitle").click(function(){
                    $.getJSON("/yuanzhan/jkd/News/checkNewsTitle", { title:$("#title").val(),id:"<?php echo ($info["id"]); ?>"}, function(json){
                        $("#checkNewsTitle").css("color",json.status==1?"#0f0":"#f00").html(json.info);
                    });
                });
                $(".submit").click(function(){
                    if($('#url1').val() == null || $('#url1').val() == ''){
                        popup.error('请上传新闻图片！');
                    }else{
                        content.sync();
                        commonAjaxSubmit();
